...
local function getset__index(table, key)
    ...
    -- if an old__index exists, use that
...
local function getset__newindex(table, key, value)
    ...
    -- use the old__newindex from the previous metatable next
    if gs.old__newindex then
        if type(gs.old__newindex) == "function" then
            gs.old__newindex(table, key, value)
        else
            gs.old__newindex[key] = value
        end
        return
    end
